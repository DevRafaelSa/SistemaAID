...
from valida_cpf_cnpj.validators import is_valid_cnpj, cnpj_random_generator
import re
...
class CNPJ(object):
    ...
    def __repr__(self):
        if not self.is_valid():
            return str(
                'Inválido{}(raw_input={})'.format(type(self).__name__, self.raw_input)
            )
        else:
            return str(
    ...
    def __lt__(self, other):
        ...
        if invalid is not None:
            raise ValueError('CNPJ inválido: %r' % invalid)
...
def cnpj_to_python(value):
    if value in [None, '']:
        ...
        raise TypeError("Não é possível converter %s para CNPJ." % type(value).__name__)

    return cpf_number


if __name__ == '__main__':
    cnpf_invalid = CNPJ('Inválido')
